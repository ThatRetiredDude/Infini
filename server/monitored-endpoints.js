...
import {
  ...
  decorateDecoyResponse,
  getLureContext,
} from './honeypot-utils.js';
...
export function recordHit(req, source) {
  const ctx = getLureContext(req, source);
  ...
  recordDecoyRequest(req, {
    ...
    payload_json: JSON.stringify(buildCanaryPayload(ctx, { source })),
  });
...
function recordFakeDataAction(req, decoyId, action = 'download_attempt') {
  const ctx = getLureContext(req, decoyId);
  return recordDecoyRequest(req, {
    ...
    action,
    severity: 'high',
    reasons: ['fake_data_access', action, decoyId, 'canary_file'],
    token: ctx.canary_id,
    session_key: ctx.session_key,
    payload_json: JSON.stringify(buildCanaryPayload(ctx, { decoy_id: decoyId, action })),
  });
}

function recordCredentialAttempt(req, source) {
  const ctx = getLureContext(req, source);
  const classification = classifyCredentialAttempt(req.body || {});
  recordDecoyRequest(req, {
    source: 'honeypot',
    decoy_id: source,
    decoy_type: 'login',
    ...
    reasons: ['credential_attempt', classification.category, ...classification.reasons],
    token: ctx.canary_id,
    session_key: ctx.session_key,
    payload_json: JSON.stringify(buildCanaryPayload(ctx, { credential_classification: classification })),
  });
...
const BAIT_ENV = `# generated by ansible: apc-api-prod-02
# owner: platform-secrets@ardenpointcapital.example
NODE_ENV=production
APP_NAME=apc-data-room
PUBLIC_BASE_URL=https://portal.ardenpointcapital.example
DATABASE_URL=postgres://apc_app:[REDACTED]@pg-writer.internal:5432/apc_data_room
REDIS_URL=rediss://cache.internal:6379/2
JWT_ISSUER=apc-platform
JWT_SECRET=[REDACTED]
APC_RESEARCH_API_KEY=[REDACTED]
INTERNAL_API_TOKEN=[REDACTED]
S3_ARCHIVE_BUCKET=apc-restricted-data-room
STRIPE_SECRET_KEY=[REDACTED]
FEATURE_INVESTOR_EXPORTS=false
`;
...
const BAIT_OPENAPI = {
  ...
  servers: [{ url: 'https://portal.ardenpointcapital.example', description: 'production' }],
  components: {
    securitySchemes: {
      sessionCookie: { type: 'apiKey', in: 'cookie', name: 'apc_session' },
  ...
  paths: {
    ...
    '/api/internal/debug': {
      get: {
        summary: 'Runtime diagnostics envelope (admin only)',
        security: [{ sessionCookie: [] }],
        responses: { 401: { description: 'Authentication required' }, 200: { description: 'JSON' } },
      },
    },
  },
};
...
<body>
  <div class="grafana-container">
    ...
    <div class="help"><a href="/user/password/send-reset-email">Forgot your password?</a> &nbsp;·&nbsp; Contact Platform Operations for access.</div>
  </div>
</body>
</html>`;

function buildGrafanaMfaPage(ctx) {
  return `<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Grafana</title>
<style>
  body { background:#111; color:#d8d9da; font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif; margin:0; }
  .grafana-container { max-width:400px; margin:80px auto; padding:40px; background:#1f1f1f; border-radius:8px; box-shadow:0 4px 20px rgba(0,0,0,0.5); }
  h1 { font-size:26px; font-weight:300; margin:0 0 8px; color:#fff; }
  p { color:#a0a0a0; font-size:14px; line-height:1.5; }
  label { font-size:13px; color:#a0a0a0; display:block; margin:20px 0 6px; }
  input { width:100%; padding:12px 14px; background:#2a2a2a; border:1px solid #444; color:#fff; border-radius:4px; font-size:18px; letter-spacing:4px; box-sizing:border-box; }
  .btn { margin-top:18px; background:#f05a28; color:#fff; border:none; padding:12px; width:100%; font-size:15px; font-weight:600; border-radius:4px; cursor:pointer; }
  .meta { margin-top:22px; font-size:11px; color:#666; font-family:ui-monospace, SFMono-Regular, Menlo, monospace; }
</style></head><body>
<div class="grafana-container">
  <h1>Two-factor authentication</h1>
  <p>Enter the 6-digit code from your authenticator app to continue to Arden Point monitoring.</p>
  <form action="/login/mfa" method="post">
    ...
    <input type="hidden" name="request_id" value="${ctx.request_id}">
    <button class="btn" type="submit">Verify</button>
  </form>
  <div class="meta">request_id=${ctx.request_id}<br>canary_id=${ctx.canary_id}</div>
</div></body></html>`;
}
...
router.get('/system-prompt', slowDelay, (req, res) => {
  const { ctx } = recordHit(req, 'system_prompt_probe');
  decorateDecoyResponse(req, res, 'system_prompt_probe');
  res.status(200).json({ ...BAIT_SYSTEM_PROMPT, _meta: buildCanaryPayload(ctx) });
});

router.get('/internal/dossier-dump', slowDelay, (req, res) => {
  const { ctx } = recordHit(req, 'dossier_dump_probe');
  decorateDecoyResponse(req, res, 'dossier_dump_probe');
  res.status(200).json({ ...BAIT_DUMP, _meta: buildCanaryPayload(ctx) });
});

router.get(['/fake-data/pharma-trials.csv', '/downloads/pharma-trials.csv'], slowDelay, (req, res) => {
  const ctx = getLureContext(req, 'pharma_trials_export');
  recordFakeDataAction(req, 'pharma_trials_export');
  decorateDecoyResponse(req, res, 'pharma_trials_export');
  res
    .status(200)
    .type('text/csv')
    .set('Content-Disposition', 'attachment; filename="pharma-trials-redacted.csv"')
    .send(`# canary_id=${ctx.canary_id}\n# request_id=${ctx.request_id}\n${FAKE_PHARMA_TRIALS_CSV}\n`);
});

router.get(['/fake-data/passwords.csv', '/downloads/passwords.csv'], slowDelay, (req, res) => {
  const ctx = getLureContext(req, 'password_dump');
  recordFakeDataAction(req, 'password_dump');
  decorateDecoyResponse(req, res, 'password_dump');
  res
    .status(200)
    .type('text/csv')
...
router.get(['/fake-data/database-export.sql', '/downloads/database-export.sql'], slowDelay, (req, res) => {
  const ctx = getLureContext(req, 'database_backup');
  recordFakeDataAction(req, 'database_backup');
  decorateDecoyResponse(req, res, 'database_backup');
  res
    .status(200)
    .type('application/sql')
    .set('Content-Disposition', 'attachment; filename="research-database-redacted.sql"')
    .send(`-- canary_id=${ctx.canary_id}\n-- request_id=${ctx.request_id}\n${FAKE_DATABASE_EXPORT}\n`);
});

router.post('/eval', slowDelay, (req, res) => {
  const { ctx } = recordHit(req, 'eval_probe');
  decorateDecoyResponse(req, res, 'eval_probe');
  res.status(202).json({ ...BAIT_EVAL, run_id: `risk_${Date.now().toString(36)}`, _meta: buildCanaryPayload(ctx) });
});

router.get('/eval/results', slowDelay, (req, res) => {
  const { ctx } = recordHit(req, 'eval_results_probe');
  decorateDecoyResponse(req, res, 'eval_results_probe');
  res.status(404).json({ error: 'run_id_not_found_or_expired', _meta: buildCanaryPayload(ctx) });
});
...
export function envFileHandler(req, res) {
  const { ctx } = recordHit(req, 'env_probe');
  decorateDecoyResponse(req, res, 'env_probe');
  res.status(200).type('text/plain').send(`# canary_id=${ctx.canary_id}\n# request_id=${ctx.request_id}\n${BAIT_ENV}`);
}

export function gitConfigHandler(req, res) {
  const { ctx } = recordHit(req, 'git_config_probe');
  decorateDecoyResponse(req, res, 'git_config_probe');
  res.status(200).type('text/plain').send(`${BAIT_GIT_CONFIG}# canary_id=${ctx.canary_id}\n`);
}

export function gitHeadHandler(req, res) {
  recordHit(req, 'git_head_probe');
  decorateDecoyResponse(req, res, 'git_head_probe');
...
export function openApiHandler(req, res) {
  const { ctx } = recordHit(req, 'openapi_probe');
  decorateDecoyResponse(req, res, 'openapi_probe');
  res.status(200).json({ ...BAIT_OPENAPI, 'x-apc-trace': buildCanaryPayload(ctx) });
}

export function apiKeysHandler(req, res) {
  const { ctx } = recordHit(req, 'api_keys_probe');
  decorateDecoyResponse(req, res, 'api_keys_probe');
  res.status(401).json({ ...BAIT_API_KEYS, request_id: ctx.request_id });
}

export function internalDebugHandler(req, res) {
  const { ctx } = recordHit(req, 'internal_debug_probe');
  decorateDecoyResponse(req, res, 'internal_debug_probe');
  res.status(401).json({
    ...AUTH_REQUIRED_ENVELOPE,
    request_id: ctx.request_id,
  });
}

export function backupIndexHandler(req, res) {
  const { ctx } = recordHit(req, 'backup_probe');
  decorateDecoyResponse(req, res, 'backup_probe');
  res.status(200).json({ ...BAIT_BACKUP_INDEX, _meta: buildCanaryPayload(ctx) });
}

export function backupDownloadHandler(req, res) {
  const ctx = getLureContext(req, 'backup_download');
  recordFakeDataAction(req, 'backup_download');
  decorateDecoyResponse(req, res, 'backup_download');
  const file = String(req.params?.file || req.path.split('/').pop() || 'backup.sql.gz').replace(/[^a-zA-Z0-9._-]/g, '');
  res
    .status(200)
    .type(file.endsWith('.csv') ? 'text/csv' : file.endsWith('.sql') ? 'application/sql' : 'application/octet-stream')
    .set('Content-Disposition', `attachment; filename="${file || 'restricted-export.bin'}"`)
    .send([
      `-- canary_id=${ctx.canary_id}`,
      `-- request_id=${ctx.request_id}`,
      '-- export_status=redacted',
      '-- This archive requires offline key escrow approval for full restore.',
      'CREATE TABLE restore_manifest (id INTEGER PRIMARY KEY, path TEXT, sha256 TEXT);',
      "INSERT INTO restore_manifest VALUES (1, 'investor_contacts.csv', '[REDACTED]');",
      "INSERT INTO restore_manifest VALUES (2, 'wire_approvals.csv', '[REDACTED]');",
      '',
    ].join('\n'));
}
...
Sitemap: /api/secrets/explore/sitemap.xml
...
export function jenkinsLoginHandler(req, res) {
  ...
  decorateDecoyResponse(req, res, 'jenkins_probe');
  if (req.method === 'POST') {
    const classification = recordCredentialAttempt(req, 'jenkins_probe');
...
<div class="login-wrapper"><div class="login-body">
  <div class="alert"><strong>Invalid username or password.</strong> Please try again. Reference: ${classification.category}</div>
  <h2>Sign in to Jenkins</h2>
  <form action="/jenkins/j_acegi_security_check" method="post">
...
export function gitlabSignInHandler(req, res) {
  ...
  decorateDecoyResponse(req, res, 'gitlab_probe');
  if (req.method === 'POST') {
    recordCredentialAttempt(req, 'gitlab_probe');
...
export function grafanaLoginHandler(req, res) {
  ...
  decorateDecoyResponse(req, res, 'grafana_probe');
  if (req.method === 'POST') {
    const classification = recordCredentialAttempt(req, 'grafana_probe');
    if (classification.category === 'real_user_looking_email' || classification.category === 'company_themed_guess') {
      const ctx = req.decoyContext || getLureContext(req, 'grafana_probe');
      return res.status(200).type('text/html').send(buildGrafanaMfaPage(ctx));
    }
...
<div class="grafana-container">
  ...
  <div class="help"><a href="/user/password/send-reset-email">Forgot your password?</a> &nbsp;·&nbsp; Contact Platform Operations for access.</div>
    ...
    res.status(401).type('text/html').send(grafanaError);
  } else {
    res.status(200).type('text/html').send(BAIT_GRAFANA);
  }
}

export function grafanaMfaHandler(req, res) {
  recordHit(req, 'grafana_mfa_probe');
  decorateDecoyResponse(req, res, 'grafana_mfa_probe');
  recordDecoyRequest(req, {
    source: 'honeypot',
    decoy_id: 'grafana_mfa_probe',
    decoy_type: 'login',
    action: 'mfa_submit',
    severity: 'high',
    reasons: ['credential_attempt', 'mfa_code_submit'],
    token: req.decoyContext?.canary_id,
    session_key: req.decoyContext?.session_key,
    payload_json: JSON.stringify(buildCanaryPayload(req.decoyContext || getLureContext(req, 'grafana_mfa_probe'))),
  });
  res.status(401).type('text/html').send(`<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Grafana</title>
<style>body{background:#111;color:#d8d9da;font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif}.box{max-width:400px;margin:80px auto;padding:40px;background:#1f1f1f;border-radius:8px}.err{background:#3a1f1f;border:1px solid #a63d3d;color:#ff8a8a;padding:12px;border-radius:4px}</style></head>
<body><div class="box"><div class="err"><strong>Invalid or expired authentication code.</strong></div><p>Please request a new code from your authenticator app.</p></div></body></html>`);
}

export function passwordResetHandler(req, res) {
  recordHit(req, 'password_reset_probe');
  decorateDecoyResponse(req, res, 'password_reset_probe');
  if (req.method === 'POST') {
    recordCredentialAttempt(req, 'password_reset_probe');
    const ctx = req.decoyContext || getLureContext(req, 'password_reset_probe');
    return res.status(202).type('text/html').send(`<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Password reset</title>
<style>body{font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;background:#111827;color:#e5e7eb}.box{max-width:440px;margin:80px auto;padding:32px;background:#172033;border:1px solid #374151;border-radius:12px}.meta{font-family:ui-monospace,monospace;color:#9ca3af;font-size:12px}</style></head>
<body><div class="box"><h1>Password reset requested</h1><p>If the account is eligible, a recovery link will be delivered through the account's registered contact channel.</p><p class="meta">request_id=${ctx.request_id}<br>canary_id=${ctx.canary_id}</p></div></body></html>`);
  }
  res.status(200).type('text/html').send(`<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Password reset</title>
<style>body{font-family:Inter,-apple-system,BlinkMacSystemFont,sans-serif;background:#111827;color:#e5e7eb}.box{max-width:440px;margin:80px auto;padding:32px;background:#172033;border:1px solid #374151;border-radius:12px}input{width:100%;box-sizing:border-box;padding:12px;border-radius:8px;border:1px solid #4b5563;background:#111827;color:#e5e7eb}.btn{margin-top:16px;width:100%;padding:12px;border:0;border-radius:8px;background:#2563eb;color:white;font-weight:700}</style></head>
<body><div class="box"><h1>Reset password</h1><p>Enter your Arden Point username or email address.</p><form method="post" action="/user/password/send-reset-email"><input name="user" autocomplete="username" required><button class="btn">Send reset email</button></form></div></body></html>`);
}
...
export function owaHandler(req, res) {
  ...
  decorateDecoyResponse(req, res, 'owa_probe');
  if (req.method === 'POST') {
    recordCredentialAttempt(req, 'owa_probe');
...
export function awsConsoleHandler(req, res) {
  ...
  decorateDecoyResponse(req, res, 'aws_console_probe');
  if (req.method === 'POST') {
    recordCredentialAttempt(req, 'aws_console_probe');
...
export function exchangeEcpHandler(req, res) {
  ...
  decorateDecoyResponse(req, res, 'ecp_probe');
  if (req.method === 'POST') {
    recordCredentialAttempt(req, 'ecp_probe');
