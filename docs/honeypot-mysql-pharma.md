...
- Includes a `password_backup` table (poorly named by a sloppy admin) containing up to 2 million realistic-looking high-cost bcrypt hashes to waste attacker cracking time and GPU budget.
...
- **password_backup**: up to 2 million rows of `email`, high-cost bcrypt `hash`, `group`, `working_group`, `user_level`. (Table named `password_backup` on purpose — exactly what a careless IT admin would create and leave exposed.) `working_group` values reflect real pharma teams (Regulatory Affairs, Clinical Operations, Quality Assurance, etc.). `user_level` reflects typical medical-office roles (physician, clinical_pharmacist, regulatory_specialist, etc.).
